...
## v2 додаткові ЛІЧИЛЬНІ критерії (граматика)
- ШАРНІСТЬ: кожна flagship-секція ≥5 шарів (порахуй по motion-score рядку стека і коду), службова ≥3.
- ДИСТАНЦІЇ: партитура дистанцій існує і сусідні секції не повторюють план; обʼєкт повністю ≤2.
- ШВИ: жоден спосіб переходу не вжитий двічі; є ≥1 theme-flip.
- ЕКСПОЗИЦІЯ: одиниця «акт+медіа+факт-абзац» дотримана; тон актів єдиний; бренд-рефрен hero→footer.
- Перевір state/*.yaml: всі секції пройшли стадії (прототипи/вибір/verify) — пропущена стадія = P0 процесу.
