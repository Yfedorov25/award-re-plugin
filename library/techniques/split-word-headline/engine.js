...
(function (global) {
  ...
  function create(target, options) {
    ...
    // forceMotion (dual-platform opt-in): the original component disabled travel <=560px
    // because it had no mobile branch. The dual wrapper OWNS a mobile branch (auto-play),
    // so it passes forceMotion:true to keep the animation on phones. reduced-motion is
    // still honored (accessibility). Default (no flag) = original desktop-only behavior.
    var narrow = !options.forceMotion && global.matchMedia && global.matchMedia('(max-width: 560px)').matches;
