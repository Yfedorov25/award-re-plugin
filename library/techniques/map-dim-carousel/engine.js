...
(function (global) {
  ...
  function create(target, options) {
    ...
    // forceMotion (dual-platform): the dual wrapper drives set(p)/go() itself, so skip the
    // <=820px static-lock and keep the live announce + rail on phones.
    ...
    // manualDrive (dual-platform): the wrapper owns the scroll driver (pin-wrap scrub),
    // so DON'T create the engine's own ScrollTrigger — expose pure set(p)/go() only.
    var trigger = null;
    if (!options.manualDrive) {
      ...
      trigger = ScrollTrigger.create(stConf);
    }
