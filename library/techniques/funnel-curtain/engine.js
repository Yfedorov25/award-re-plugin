...
(function (global) {
  ...
  /* springs ЄДИНА моушн-крива (cubic-bezier(.25,.74,.22,.99) ×72 на всьому сайті,
     D_Springs_architecture §2). Була AIR-крива (.7,0,.3,1) — не springs. */
  ...
  function create(options) {
    ...
    var opt = {
      ...
      /* springs continuous-mode (Barba): один плавний рух на кожну ногу */
      coverMs: options.coverMs != null ? options.coverMs : 720,
      uncoverMs: options.uncoverMs != null ? options.uncoverMs : 720,
    ...
    function buildPanel(mode) {
      ...
      if (mode === 'continuous') {
        /* springs-режим: центрований wordmark (ДІМ) у Victor Serif, їде з панеллю */
        var mid = doc.createElement('div');
        mid.className = 'fcr__mid';
        mid.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;' +
          'justify-content:center;pointer-events:none';
        var w = doc.createElement('span');
        w.textContent = String(opt.wordmark).replace(/\s+/g, '');
        w.style.cssText = 'font-family:"Victor Serif",Georgia,serif;font-weight:400;' +
          'font-size:clamp(64px,16vw,180px);letter-spacing:-1px;line-height:1;color:' + opt.ink;
        mid.appendChild(w);
    ...
    function play(swapFn, mode) {
      ...
      /* SPRINGS-РЕЖИМ (Barba leave→enter): БЕЗПЕРЕРВНИЙ. Один плавний рух покриття
         0→100% (springs-ease), свап РІВНО під повним покриттям, один плавний рух виходу
         100%→-100%. Жодної бренд-паузи посередині, жодної двошвидкісності — саме це
         Єгор просив «перехід на ДІМ безперервний, не ривками». */
      if (mode === 'continuous') {
        return step(panel, 'translateY(0)', opt.coverMs)
          .then(function () {
            gate.coveredT = performance.now();
            swapFn();                 /* свап у момент повного покриття */
            return delay(opt.holdMs); /* мінімальний hold, лише щоб свап відмалювався */
          })
          .then(function () { return step(panel, 'translateY(-100%)', opt.uncoverMs); })
          .then(function () {
            if (panel.parentNode) panel.parentNode.removeChild(panel);
            gate.doneT = performance.now();
            busy = false;
      ...
      /* ДВОФАЗНИЙ ритуал (AIR): вискок до паузи → білборд → докриття → свап → вихід */
