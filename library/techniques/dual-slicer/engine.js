...
(function (global) {
  ...
  function init(target, options) {
    ...
    // static fallback
    if (!options.manualDrive && (reduced || narrow || !gsap || !ScrollTrigger || !Lenis || renders.length < 2)) {
    ...
    if (CustomEase && CustomEase.create && (!CustomEase.get || !CustomEase.get('dsGlide'))) {
    ...
    function render(prog) {
      ...
      for (var i = 1; i < N; i++) {
        ...
        renders[i].style.opacity = '1';   // clip does the reveal; keep opaque (mobile path uses opacity)
    ...
    // MOBILE render (T-CROSSFADE-ACT, D_SPRINGS_video): on a narrow screen springs does NOT
    // clip-slice the render strip (the vertical seam reads unnatural) — the renders CROSSFADE
    // via opacity instead, while the coupled left line-reveal + rising inset stay identical.
    function renderMobile(prog) {
      var stripEnd = 0.72;
      // strip: crossfade each render i in over its segment (opacity), lower covers upper
      renders.forEach(function (r) { r.style.clipPath = 'none'; });
      for (var i = 1; i < N; i++) {
        var segStart = (i - 1) / handoffs * stripEnd;
        var segEnd = i / handoffs * stripEnd;
        var p = (prog - segStart) / (segEnd - segStart);
        p = p < 0 ? 0 : p > 1 ? 1 : p;
        renders[i].style.opacity = p.toFixed(3);   // crossfade in (no seam)
      }
      // LEFT column reveal — IDENTICAL to desktop (not a side-wipe, fine on mobile)
      var firstHandoffEnd = stripEnd / handoffs;
      var leftDone = firstHandoffEnd * 0.92;
      var nLines = lines.length || 1;
      lines.forEach(function (l, k) {
        var it = l.querySelector('.ds-line__i'); if (!it) return;
        var slice = leftDone / nLines;
        var ls = k * slice, le = ls + slice * 1.5; if (le > leftDone) le = leftDone;
        var p = (prog - ls) / (le - ls); p = p < 0 ? 0 : p > 1 ? 1 : p;
        it.style.transform = 'translateY(' + ((1 - p) * 112).toFixed(2) + '%)';
      });
      if (inset) {
        var s = (leftDone / nLines) * 0.5;
        var p = (prog - s) / (leftDone - s); p = p < 0 ? 0 : p > 1 ? 1 : p;
        inset.style.transform = 'translateY(' + ((1 - p) * 118).toFixed(2) + '%)';
      }
    }

    // manualDrive (dual-platform): expose render() without the engine's own pin/ScrollTrigger.
    // desktop → render (clip-slice strip); mobile → renderMobile (crossfade strip).
    if (options.manualDrive) {
      // prime renders for whichever path: give each a base opacity so crossfade works
      renders.forEach(function (r, i) { r.style.willChange = 'clip-path, opacity'; r.style.opacity = i === 0 ? '1' : '0'; });
      render(0); try { global.__LAB_OK__ = true; } catch (e) {}
      return { manual: true, render: render, renderMobile: renderMobile, count: renders.length, destroy: function () {} };
    }
