---
id: floorplan-smarts--ever-band
name: "Floorplan SMARTS ever-band (EVER daylight sky skin; ported from Claude Design 03; a near-full daylight facade + ONE tracking tooltip and a hairline measure-band that glide between floors; full building->floor->unit drill on the real 54-apartment object)"
role: floorplan
kind: combo
status: base
family: floorplan-smarts
palette: ever-daylight
entry:
...
meaning:
  what: "The conversion-core floorplan for the real SMARTS object (one entrance, 54 apartments), ported from a Claude Design composition onto our real engine. PALETTE = EVER daylight sky-to-ground gradient (ink #1c2227, terracotta #b9633c). Fraunces + Inter. FULL drill: L1 the real facade with hover floor-bands (the OFF-facade affordance never covers the building) -> click a floor -> L2 the clean redrawn numbered floorplate beside a persistent unit-list (nr/type/area/price/status, live free-counts, spec always visible) -> click a unit -> L3 the calm veil flat-detail. Controlled-click; 0 pins; NO-WebGL."
  ...
  signature: "The facade fills the stage in daylight; ONE tracking tooltip + a hairline MEASURE BAND (two glowing lines at the active floor top+bottom) glide between floors (expo.out). Minimal, surgical."
...
# floorplan-smarts--ever-band

Ported from Claude Design onto our real engine (FBANDS L1 -> numbered-floorplate-select L2 -> veil-flat-detail-reveal L3) with the real 54-unit smarts data. PALETTE = EVER daylight sky-to-ground gradient (ink #1c2227, terracotta #b9633c). Fraunces + Inter.
