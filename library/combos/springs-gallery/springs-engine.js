...
(async () => {
  ...
  /* Z-ПОРЯДОК ОБГОРТОК ПІСЛЯ ПРУНІНГУ (S17, s9180 корінь #2, пастка 59):
     прунінг вкладених секцій (пастка 40) робить бг-секцію (place-bg =
     .l-nature-bg, вміст z-index:2) СИБЛІНГОМ секції з контентом (nature =
     .l-nature, каптіон z:3). Обидві обгортки sk-vp = z:auto absolute → їхній
     вміст порівнюється НЕ за z, а за DOM-порядком → бг малюється ПОВЕРХ
     каптіона. Точковий фікс: підняти ЛИШЕ `nature` над `place-bg` (не
     глобальний z-max — S17 довів регресом: глобальний z-max підняв wellness/
     spa-обгортку над nature-leaf на s7194 12→53, net-negative по зоні). Лише
     ця пара — каптіон s9180 26.9→3.69, анкери 0 регресу; s7194 недоторканий. */
  {
    const natW = wrappers['nature'], bgW = wrappers['place-bg'];
    if (natW && bgW) {
      const bgZ = parseInt(getComputedStyle(bgW.firstElementChild || bgW).zIndex, 10);
      const base = Number.isFinite(bgZ) ? bgZ : 2;
      natW.style.position = getComputedStyle(natW).position === 'static' ? 'relative' : getComputedStyle(natW).position;
      natW.style.zIndex = String(base + 1); /* nature над бг-сиблінгом */
    }
  }
  ...
  function buildOwn() {
    ...
    for (const b of bound) {
         ...
         height реплеїться напряму, scale тут зіпсував би дітей */
      {
        ...
        /* SCALE лише коли розмір РЕАЛЬНО анімується вздовж кривої (спани-
           кружечки wellness пульсують w/h). КОНСТАНТНИЙ розмір, що не
           збігається з каркасним, = СТРУКТУРНА розбіжність (S17 форк B:
           section-root gallery має curve.h=2880 constant, а nat.h=900 —
           пінна секція в live виміряна на 900, справжній контент 2880 →
      ...
      const mk = (x, sKey) => {
        /* sw/sh ЛИШЕ якщо dim реально варіюється вздовж кривої (b.wVaries/
           hVaries) — constant-size розбіжність каркас↔live = структурна,
           не анімація (S17: section-root ×3.2 баг) */
        const sw = b.wVaries && b.nat.w > 1 && x.w > 0 ? x.w / b.nat.w : 1;
        const sh = b.hVaries && b.nat.h > 1 && x.h > 0 ? x.h / b.nat.h : 1;
