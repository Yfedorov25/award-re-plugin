...
acceptance:
  ...
  - "порядковий каскад (data-brv-lines): параграф проявляється РЯДОК за рядком (+60ms/line-index — канон AIR word-wrap); спани-обгортки РОЗГОРТАЮТЬСЯ назад у чистий текст після reveal (нуль постійної ваги в DOM); вкладена розмітка = чесний фолбек на поблоковий"
...
за offsetTop у рядки, кожен рядок стартує +60мс; після завершення
спани розгортаються назад у чистий текст (resize-стійкість, нуль ваги).
