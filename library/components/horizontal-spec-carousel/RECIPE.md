...
acceptance:
  ...
  - "driven by arrows, drag/swipe (threshold max(40px, 18% of a card); pure click never steps; drag move/up tracked on window so a drag leaving the viewport still ends cleanly + the spec-row never blanks on a non-step), horizontal wheel intent, and Left/Right keys; the stage is focusable"
