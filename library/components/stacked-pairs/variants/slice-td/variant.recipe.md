...
variant: slice-td
name: "stacked-pairs / slice-td"
status: official
mode: pinned-slice
dir: td
source:
  grammar: "QUADRO slide-lab — frs13 stacked pairs + SAISEI slice reveal (TOP->DOWN): the same frs7 text-left / render-right composition and the same ONE-Lenis engine as the base, but the reveal is a PINNED, scrubbed, SAISEI-timed clip-wipe instead of the base's non-pinned fade+rise; the only difference from slice-lr is the clip axis"
  recording: "apps/quadro/public/slide-lab/frs13-stacked-slice-td.html"
  registry_ref: []
meaning:
  lands: "Each pair arrives and HOLDS you (the section is pinned): the text seats first, then a clear quiet PAUSE with the text alone, then the big render on the right CLIP-WIPES open slowly from the TOP edge downward (the visible region grows down the frame), then it holds settled before the pin releases to the next pair. A deliberate, controlled, graceful reveal whose pace does not depend on how fast you flick — the SAISEI read, not the base's quick in-view fade."
overrides:
  ...
  clip: "TOP->DOWN — the render reveals via clipPath inset(0 0 B% 0) with B 100->0, so the visible region starts at the TOP edge and grows DOWNWARD. Do NOT invert it (do NOT clip the top). (slice-lr instead clips the RIGHT, inset(0 R% 0 0), left->right.)"
when_pick_this: "The same text-left / render-right pair composition, but you want each pair to PIN and reveal on a slow, controlled SAISEI beat (text -> pause -> render clip-wipes open) instead of the base's quick in-view fade+rise, and you want the render to wipe open TOP->DOWN. Pick the BASE (stacked-pairs) for the calm pin-LESS vertical magazine rhythm; pick slice-lr for the same pinned slice revealed LEFT->RIGHT."
files: [variant.recipe.md, params.json, variant.lab.html]
verify: "variant.lab.html#__LAB_OK__"
---

# stacked-pairs / slice-td — variant delta (pinned-slice mode)
...
> **clip-path WIPE** that opens **TOP->DOWN**. Recorded 1:1 from owner-approved
> `frs13-stacked-slice-td.html` (gate-clean).
...
> documents `mode:pinned-slice` + `dir:td`), NOT a `params.json`-only knob diff
> over the base and NEVER a forked `component.js`. The ONLY difference from
> `slice-lr` is the clip axis.
...
- **Clip axis TOP->DOWN.** The render reveal uses
  `clipPath: inset(0 0 B% 0)` with `B = (1-rev)*100` (100 -> 0), so the visible
  region **starts at the TOP edge and grows DOWNWARD** — a top-to-down wipe.
  **Do not invert this** (do not clip the top: `inset(B% 0 0 0)` would reveal
  bottom->up, which is the wrong direction). The sibling `slice-lr` clips the
  RIGHT instead.
...
difference vs `slice-lr` is the clip axis.

## How it is recorded
`variant.lab.html` is self-contained and runnable: it authors the frs11/frs13
...
(clip axis `inset(0 0 B% 0)`, top->down). Renders resolve through the `renders`
