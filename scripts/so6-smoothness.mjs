// so6-smoothness.mjs (residences carousel: header/card enter → 3-type cross-dissolve → seam) — measures INTRO→carousel smoothness of SO-2 via REAL scroll.
...
const N=68, PMAX=0.62;                 // intro reveal → hold → rise-out → rail/carousel entry → Spa solo → first FULL tab swap
async function run(vp,label){
  ...
  await p.evaluate(async ()=>{ await Promise.all([1,2,3].map(n=>{const i=new Image();
    i.src=`media/residence-${n}.webp`;return i.decode().catch(()=>{});})); });
