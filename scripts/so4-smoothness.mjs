// so4-smoothness.mjs (Place tail → colour-flip → map settle → hover-pin layer) — measures INTRO→carousel smoothness of SO-2 via REAL scroll.
...
const N=70, PMAX=1.0;                 // intro reveal → hold → rise-out → rail/carousel entry → Spa solo → first FULL tab swap
async function run(vp,label){
  ...
  await p.evaluate(async ()=>{ const i=new Image(); i.src='media/map.webp'; await i.decode().catch(()=>{}); });
...
// SO-4 has the DESIGNED sharp colour-flip Place→cream Location (+87 luma in one gesture window,
// S24-verified springs "дихання ритму") = inherently a big full-screen crossfade; ceilings desktop
// 28 / mobile 30. sawtooth===0 stays the hard gate (the flip is a monotonic ramp, not a pop).
for(const [vp,lbl,ceil] of [[{width:1440,height:900},'desktop',28],[{width:390,height:844},'mobile',30]]){
