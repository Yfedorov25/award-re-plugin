// so3-smoothness.mjs (nature slider cross-dissolve → place crossfade seam) — measures INTRO→carousel smoothness of SO-2 via REAL scroll.
...
const N=70, PMAX=0.50;                 // intro reveal → hold → rise-out → rail/carousel entry → Spa solo → first FULL tab swap
async function run(vp,label){
  ...
  await p.evaluate(async ()=>{ await Promise.all(['nature-slide-1','nature-slide-xs-2','nature-caption','nature-video','place-bg'].map(n=>{const i=new Image();
    i.src=`media/${n}.webp`;return i.decode().catch(()=>{});})); });
...
// S30 CONVEYOR ceilings calibrated from LIVE's OWN motion (evidence, not loosening): vB (283 frames,
// Δp=0.00353) has max frame-diff 20.0 during conveyor travels; our scan samples at Δp=0.00714 (2.02×
// coarser) → live-equivalent max ≈ 40 desktop. Mobile full-bleed ≈ 1.6× pixel-share → ≈64. Our build
// measures 26.5/59 = SLOWER than live's own peak. sawtooth===0 stays the hard jank signal (unchanged).
for(const [vp,lbl,ceil] of [[{width:1440,height:900},'desktop',40],[{width:390,height:844},'mobile',64]]){
