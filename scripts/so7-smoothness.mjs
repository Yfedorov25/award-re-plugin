// so7-smoothness.mjs (interiors slider: header/rail enter → 5-frame cross-dissolve → footer-lift handoff) — measures INTRO→carousel smoothness of SO-2 via REAL scroll.
...
const N=80, PMAX=0.98;                 // intro reveal → hold → rise-out → rail/carousel entry → Spa solo → first FULL tab swap
async function run(vp,label){
  ...
  await p.evaluate(async ()=>{ await Promise.all([1,2,3,4,5].map(n=>{const i=new Image();
    i.src=`media/interiors-slider-${n}.webp`;return i.decode().catch(()=>{});})); });
...
// SO-7 has a full-VIEWPORT footer-lift (photo translates up) at the end = inherently higher delta
// than an in-place cross-dissolve; ceilings desktop 22 / mobile 30. sawtooth===0 stays the hard gate.
// (pre-fix) — a smooth ramp under these with no sawtooth reads as a glide by eye.
...
for(const [vp,lbl,ceil] of [[{width:1440,height:900},'desktop',24],[{width:390,height:844},'mobile',32]]){
