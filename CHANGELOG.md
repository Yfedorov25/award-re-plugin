# 1.7.0 — Напрямки задач (track) (2026-06-18)
/award-re:task тепер додає поле track (web/3d/ads/control) у кожну задачу — напрямок.
Дашборд control фільтрує задачі по напрямках. Так список зрозумілий: сайти / 3D / реклама / платформа.

# 1.6.0 — Змістовні сповіщення в Telegram (2026-06-17)
notifyDone тепер несе `summary` (що саме зроблено) + `next` (наступні кроки) — не голе
«готово». Нова дія `progress`: апдейт/питання ПО ХОДУ роботи (на відміну від clarify —
неоднозначність ДО старту). Сесія звітує юзеру змістовно: «✅ Готово + Що зроблено +
Далі», «🔵 Апдейт», «❓ Питання по ходу». Inbox-режим task.md оновлено під це.

# 1.5.0 — Мітки сесій: кнопки бота = СЕСІЇ (2026-06-17)
Кнопки Telegram-бота тепер = СЕСІЇ (web3 / towns / control / any), не сайти. Одна сесія
володіє однією міткою й може покривати кілька сайтів (web3 = smarts+nahirna+quadro).
Нова команда **`/award-re:session <мітка>`** закріплює мітку цього чату (у памʼяті сесії,
не файл — towns і web3 в одному репо). Далі inbox розгрібає лише задачі своєї мітки.
Неймінг-конвеншн: коротка kebab-мітка на сесію. Якщо мітку не задано — СПИТАЙ, не вгадуй.

# 1.4.0 — Маршрутизація задач по сесіях (2026-06-17)
Бот після прийому ПИТАЄ кнопками, в яку сесію направити задачу (smarts-mobile / nahirna /
quadro / control / будь-яка). Inbox-режим тепер розгрібає ТІЛЬКИ свою мітку:
`{"action":"inbox","session":"<мітка>"}` → рядки з цією міткою або `any`. Нерозподілені
(`session=""`) не чіпає. Так задача «прикріплена» до сесії — кожен чат бачить лише своє.

# 1.3.0 — Telegram task-бот + inbox clarify-режим (2026-06-17)
Бойова частина системи control: задачі з Telegram (голос/текст) у чергу, розгрібання в
сесії, сповіщення назад. Команда `/award-re:task` отримала режим **INBOX**:
- розгрібає чергу inbox (control БД) → кристалізує у control/tasks/*.md → виконує → notifyDone.
- **clarify-дисципліна (виправлення бага):** неоднозначну задачу НЕ «приймати» наосліп —
  слати юзеру питання в Telegram (🟡 Потрібне уточнення), не fake-accept. Ясну — виконати
  + ✅ Готово. Бот каже лише «отримав, у черзі», прийняття = рішення в сесії.
- notifyDone ТІЛЬКИ після реального виконання+верифікації (звіт ≠ зроблено).
(Версія бампнута з 1.2.0 окремо, бо clarify-зміни спершу пішли в той самий номер і
`plugin update` їх не перевстановив — порівнює номер, не вміст.)
