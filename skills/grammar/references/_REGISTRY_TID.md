...
| T-119 | карусель з shader-дисторсією | Фото-карусель з UV-distortion на скролі (AIR /about, єдиний WebGL сайту; має DOM-fallback) ⚠️ виправлення №8 (дозйомка 2026-07-06): живий DESKTOP = slide-push треком з білим ґепом ~15-20px + внутрішній контрпаралакс кадру ~1.0-1.2s air-ease, стрілки = cursor-зони (T-432), видимого морфу немає — кадр live-archive/air/recon-20260706/carousel-midslide.jpg | AIR(a) §3 · REG §5 · live recon | M | T-110 | 🔴 → slide-push + inner-parallax (без WebGL) |
...
| T-207 | бренд-3D-обʼєкт наскрізний | Абстрактна скульптура як нитка ідентичності крізь сайт (AIR спіраль/сфера · SP хромові камені · ERA граніт) ✅ РОЗКРИТО 2026-07-06: у AIR це публічні Vimeo-лупи `background=1` (homepage_1/2 білі спіралі, homepage_3 чорна метал, about_1; + мобільні _m; каталог live-archive/README) — обертання ВСЕРЕДИНІ відео, секція лише пінить/дрейфить контейнер | AIR(v) · oEmbed recon | L | T-311, T-305, T-508 | ✅ живі Vimeo embeds (1-в-1 оригінал) |
...
| T-322 | text-blur-reveal каскад | ОСНОВНИЙ reveal-канон AIR: текст в'їжджає filter blur(10px)→0 + opacity, БЕЗ y-зсуву, once; секція, досягнута стрибком/мідскролом = settled МИТТЄВО (Д4б). **ЖИВИЙ CSS (знято з global.css 2026-07-06): `transition: filter 1s cubic-bezier(.25,.74,.22,.99)` + `delay: line-index × 60ms`** — 1s air-ease, каскад ПОРЯДКОВИЙ (кожен рядок +60мс; = закон «stagger 60ms/line» з §0). Медіа-двійник `.animation--image-in`: blur(20px), ті самі 1s/ease. ⚠️ конституційне тертя: доктрина D4-винятку в комбо капнула 0.3s — оригінал живе на 1s; канон-темп = вердикт власника | AIR live-CSS + D_AIR_invest_1to1 §3.1/§6.1 (0.5-0.8s по кадрах = перцепція хвоста air-кривої) | S | T-310, T-422, T-433, T-M23 | ✅ gsap filter-твін (text-blur-reveal; двічі бойово в invest-комбо) |
...
| T-512 | center-band wipe | Нове фото розкривається горизонтальною смугою з ЦЕНТРУ старого (слайдер v3); шов свапу медіа всередині слайдера/акту; моб-родич = push-wipe T-M23. ⚠️ ЖИВИЙ КОД (2026-07-06, imageClipIn/OutVertical): НЕ «з центру» — старе колапсує до краю, нове розкривається від протилежного (polygon-кліпи 1s ease-out, межі синхронні → один шов краєм-до-краю: вперед знизу↑, назад згори↓); канон = вердикт власника | AIR(v3) слайдер + D_AIR_invest_1to1 §6.5 + живий shared.js | S | T-110, T-109 | 🟡 атом `image-slider-wipe` (спринт 2, candidate) |
| T-513 | pinned counter-slideshow | Пін «Format»: N текст-слайдів над ПОСТІЙНИМ медіа-фоном, той самий движок contentAnimation (index=floor(p·N), лічильник «N/M» свапається на старті, текст blur-swap 0.25+60ms/рядок); прогрес = ОДИН бар translateX(−100%→0) за весь пін; на hover лічильник-віджет = КУРСОР секції (cursor--counter, lerp-follower); моб = той самий пін + 2 line-ticks (T-M06), лічильник схований | AIR(a) #format (живий DOM/JS 2026-07-06) + D_AIR_architecture | S | T-510, T-512, T-322, T-M06 | 🟡 атом `pinned-counter-slideshow` (спринт 2, candidate) |
