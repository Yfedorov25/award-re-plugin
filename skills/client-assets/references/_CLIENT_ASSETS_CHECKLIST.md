...
> Джерела: [[PB_media]] §9 (13 image-types) · [[PB_visual_search]] (плити+датасет) · [[PB_interactive_map]] (карта+POI) · [[P_realestate]] (девелопер/legal) · [[PB_forms_lead_capture]] (доставка/analytics) · [[PB_discovery_strategy]] (бренд).
...
- ⬜ Назва, слоган/Big Idea (або виводимо в інтерв'ю — [[PB_discovery_strategy]])
- ⬜ Палітра бренду (або пропонуємо за [[PB_color]]) + шрифти (display + body, woff2)
- ⬜ Логотип (SVG: повний + mobile/скорочений)
- ⬜ Тон/табу, приклади що подобається (для копі)
- ⬜ Конкуренти (для research-сенсів, не візуалу)

## 2. МЕДІА (13 image-types — [[PB_media]] §9)
...
- **Вимоги:** WebP+JPEG, високої роздільності АЛЕ шрінкнуті (НЕ 8-16MB JPG — [[PB_performance]] §2), реальні рендери не стоки.

## 3. VISUAL-SEARCH (вибір квартир — якщо ЖК, [[PB_visual_search]])
...
## 4. ЛОКАЦІЯ / КАРТА ([[PB_interactive_map]])
...
## 5. КОНВЕРСІЯ / TRUST ([[PB_forms_lead_capture]], [[P_realestate]])
...
- ⬜ **Юр-дисклеймер** текст («не оферта, рендери можуть відрізнятись») — обов'язково ([[PB_footer]])
