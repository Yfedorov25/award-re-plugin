...
2. **Для КОЖНОГО рядка — СПЕРШУ оціни ясність:**
   ...
   d. ТІЛЬКИ коли РЕАЛЬНО зроблено й перевірено — ЗМІСТОВНИЙ notifyDone (НЕ голе «готово»):
      `curl ... -d '{"action":"notifyDone","secret":"<S>","id":<inbox.id>,"chat_id":"<chat_id>","task":"T-NNNN","title":"<title>","project":"<id>","summary":"<що саме зроблено, 1-5 коротких рядків через \\n>","next":"<наступні кроки / на що чекаєш, опц.>"}'`
      → боту «✅ Готово» + *Що зроблено* + *Далі*. `summary` ОБОВ'ЯЗКОВИЙ; `next` — якщо є.
3. **Питання/прогрес ПО ХОДУ роботи** (не до старту) — `action:"progress"`:
   `curl ... -d '{"action":"progress","secret":"<S>","chat_id":"<chat_id>","task":"T-NNNN","title":"<title>","body":"<що зробив/знайшов>","question":"<конкретне питання, якщо чекаєш відповіді>"}'`
   → юзеру «🔵 Апдейт» або «❓ Питання по ходу». Шли коли: зробив частину й треба рішення;
   натрапив на розвилку; хочеш показати проміжний результат. НЕ накопичуй мовчки.
4. **Бот каже лише «отримав, у черзі» — НЕ «прийнято».** Прийняття = рішення в сесії:
   notifyDone (зроблено) / clarify (неясна до старту) / progress (питання по ходу).
   НІКОЛИ не шли notifyDone наперед (звіт ≠ зроблено).
5. Кілька задач — по черзі; план, виконання, ЗМІСТОВНИЙ звіт у Telegram по кожній.
