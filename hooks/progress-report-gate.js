...
// S54, ЗАКОН Єгора: після КОЖНОЇ завершеної дії показувати ДЕ МИ в мікро/макро плані і ЩО ДАЛІ.
// Джерело плану: KAI/PLAN-MICRO-MACRO.md. Маркери блоку:
const PLAN_RE = /ДЕ МИ|мікро-план|МІКРО-ПЛАН/;
const NEXT_RE = /ДАЛІ|НАСТУПН|наступний крок/i;
// маркер хендоф-блоку
...
if (WORK_RE.test(text) && text.length > 400) {
  ...
  if (!PLAN_RE.test(text)) missing.push('блок «ДЕ МИ» — позиція в МІКРО і МАКРО плані (KAI/PLAN-MICRO-MACRO.md): які кроки закриті, який ЗАРАЗ, які попереду');
  if (!NEXT_RE.test(text)) missing.push('рядок «ДАЛІ» — що конкретно робиться наступним кроком');
  if (missing.length) {
    block(
