...
const CLAIM = new RegExp('(?<![\\p{L}\\p{N}])(готов[оаий]|1\\s*[-–]?\\s*в\\s*[-–]?\\s*1|закрит[оаий]|збігл[оа]сь|збіглися|збіг|done|closed|matches the live|pixel[-\\s]?perfect|піксель[ -]?в[ -]?піксель)(?![\\p{L}\\p{N}])', 'iu');
  ...
  const NEGATION = new RegExp('((?<![\\p{L}\\p{N}])не(?![\\p{L}\\p{N}])|нема|немає|(?<![\\p{L}\\p{N}])not(?![\\p{L}\\p{N}])|(?<![\\p{L}\\p{N}])no(?![\\p{L}\\p{N}])|fail|провал|розбіжн|відмінност|передчасн|9\\.77|діри|бреш|невалідн|розсинхрон|поки не|ще не|НЕ 1|не 1[-\\s]?в[-\\s]?1|не готов)', 'iu');
